  ...
  return (
    <footer className="relative border-t border-border/60 bg-gradient-to-b from-muted/40 via-background to-background">
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute bottom-[-240px] left-1/2 h-[420px] w-[720px] -translate-x-1/2 rounded-full bg-primary/10 blur-3xl" />
        <div className="absolute left-[18%] top-[-120px] h-[260px] w-[260px] rounded-full bg-accent/40 blur-3xl" />
      </div>
      <div className="container relative py-12">
        <div className="grid gap-12 lg:grid-cols-[1.2fr_1fr]">
          <div className="space-y-6">
            <Link
              href={`/${locale}`}
              className="inline-flex items-center gap-3 rounded-full border border-border/60 bg-background/80 px-4 py-2 text-xl font-semibold tracking-tight shadow-sm transition hover:border-primary/50 hover:text-primary"
            >
            ...
            <div className="flex flex-wrap gap-2 text-xs font-medium uppercase tracking-wide text-muted-foreground/90">
              {TECH_STACK.map((item) => (
                <span
                  key={item}
                  className="rounded-full border border-border/80 bg-background/70 px-3 py-1 shadow-sm"
                >
                  {item}
                </span>
              ))}
              ...
              <div className="mt-4 space-y-2 text-sm">
                {navigation.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="flex items-center justify-between rounded-full px-3 py-1.5 text-muted-foreground transition hover:text-foreground"
                  >
                    {item.label}
        ...
        <div className="mt-12 flex flex-col gap-4 border-t border-border/60 pt-6 text-sm text-muted-foreground md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <span
              aria-hidden
              className={cn(
                'inline-flex h-2 w-2 rounded-full bg-primary shadow-[0_0_0_4px_rgba(253,176,66,0.15)]'
              )}
            />
