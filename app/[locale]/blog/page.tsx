   ...
   <section className="grid gap-10 lg:grid-cols-[1fr_320px]">
    <div className="grid gap-8 md:grid-cols-3">
     {posts.length ? (
      ...
      <p className="col-span-full border border-dashed p-12 text-center text-muted-foreground">
       {t('noResults')}
      </p>
     )}
    </div>
    ...
    <aside className="space-y-10">
     <div className=" border bg-card p-6 shadow-md">
       ...
       {categories.map(({ name, count }) => {
        ...
        return (
         <Link
          ...
          className={
           active
            ? 'inline-flex items-center bg-primary px-3 py-1 text-sm font-medium text-primary-foreground'
            : 'inline-flex items-center border border-border px-3 py-1 text-sm text-muted-foreground transition hover:border-primary hover:text-foreground'
          }
         >
          {name}
       ...
       {tags.map(({ name, count }) => {
        ...
        return (
         <Link
          ...
          className={
           active
            ? 'inline-flex items-center bg-secondary px-3 py-1 text-xs font-semibold uppercase tracking-wide text-secondary-foreground'
            : 'inline-flex items-center border border-border px-3 py-1 text-xs font-semibold uppercase tracking-wide text-muted-foreground transition hover:border-primary hover:text-primary'
          }
         >
          #{name}
